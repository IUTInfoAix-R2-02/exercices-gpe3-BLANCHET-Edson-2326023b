...
class Personnage extends Group {
    ...
    boolean estEnCollisionJ(Personnage autrePersonnage) {
        return getBoundsInParent().contains(autrePersonnage.getBoundsInParent())
                || autrePersonnage.getBoundsInParent().contains(getBoundsInParent());
    }

    boolean estEnCollisionObs(ArrayList<Obstacle> obs){
        for(int i = 0; i < obs.size(); ++i){
            if(getBoundsInParent().contains(obs.get(i).getBoundsInParent())
                    || obs.get(i).getBoundsInParent().contains(getBoundsInParent())){
                return true;
            }
        }
